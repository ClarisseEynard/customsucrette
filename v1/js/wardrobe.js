...
var customSucrette = {
  ...
  clothe: [
    {
      category: "underwears",
      src: "https://assets3.corazondemelon.es/clothe/web/thumb_MD/74-9813e23a4194121e.png",
    },
  ],
  maxCategory: [
...
var avatar = [],
  info = [],
  clothe = [];
const requestAvatar = new XMLHttpRequest();
requestAvatar.open("GET", "../data/v1/avatar.json");
requestAvatar.responseType = "json";
requestAvatar.send();
requestAvatar.onload = function () {
  const requestInfo = new XMLHttpRequest();
  requestInfo.open("GET", "../data/v1/info.json");
  requestInfo.responseType = "json";
  requestInfo.send();
  requestInfo.onload = function () {
    const requestClothe = new XMLHttpRequest();
    requestClothe.open("GET", "../data/v1/clothe.json");
    requestClothe.responseType = "json";
    requestClothe.send();
    requestClothe.onload = function () {
...
function cargarRopa(pag = 0, subGroup = null, pagsub = 0) {
  ...
  if (input == "") {
    ...
    grupos = info.filter((v) => {
      return v.category == categoria;
    });
  } else {
    var input = normalize(input).toLowerCase();
    grupos = info.filter(function (v) {
      return normalize(v.name).toLowerCase().includes(input);
    });
  }
  ...
  if (subGroup == null) {
    for (a = pag * maxElements; a < grupos.length; a++) {
      $(".showcase").append(
        '<div class="declinable-hanger" data-category="' +
          grupos[a].category +
          '"><div class="hanger-container"></div></div>',
      );
      var leader = clothe.filter((v) => {
        return v.groupId == grupos[a].groupId;
      });
      ...
      if (leader.length > 1) {
        ...
        if (size == "little") {
          $(".hanger-container")
            .eq(elmntLooper)
            .append('<div class="hanger clickable little"><div class="item"><img></div></div>');
        } else {
          $(".hanger-container")
            .eq(elmntLooper)
            .append('<div class="hanger clickable"><div class="item"><img></div></div>');
        }
        $(".declinable-hanger").eq(elmntLooper).attr("data-groupid", grupos[a].groupId);
        $(".hanger.clickable")
          .eq(elmntLooper)
          .append(
            '<div class="declinations"><div class="panel"><div class="counter"><div class="declination-picto"></div><div> 1/' +
              leader.length +
              ' </div></div><div class="text"><div class="text-content"><p><span>Voir toutes les couleurs</span></p></div></div></div></div>',
          );
      } else {
        var seleccionado = customSucrette.clothe.filter((v) => {
          return v.src == leader[0].src;
        });
        ...
        if (seleccionado.length > 0 || cSkin == leader[0].src) {
          if (size == "little") {
            $(".hanger-container")
              .eq(elmntLooper)
              .append(
                '<div class="hanger clickable selected little"><div class="item"><img></div></div>',
              );
          } else {
            $(".hanger-container")
              .eq(elmntLooper)
              .append('<div class="hanger clickable selected"><div class="item"><img></div></div>');
          }
        } else {
          if (size == "little") {
            $(".hanger-container")
    ...
    var variacion = clothe.filter((v) => {
      return v.groupId == subGroup;
    });
    var nombre = info.filter((v) => {
      return v.groupId == subGroup;
    });
...
function cargarAvatar(page = 0) {
  ...
  for (b = page * 8; b < base.length; b++) {
    $(".showcase").append(
      '<div class="declinable-avatar" data-category="' +
        categoria +
        '"><div class="hanger-container"></div></div>',
    );
...
async function drawSucrette(size = "body") {
  ...
  if (customSucrette.avatar.customSkin == null || size == "face") {
    img =
      "https://assets3.corazondemelon.es/avatarpart/web/thumb_MD/" +
      customSucrette.avatar.skin +
      ".png";
  } else {
  ...
  if (size == "body" || size == "small") {
    img = img.replace("_MD", "");
  } else if (size == "face" || size == "big") {
    img = img.replace("_MD", "_HD");
  }
  ...
  // Cargar cejas
  var img =
    baseIMG + customSucrette.avatar.eyebrow + "_" + customSucrette.avatar.hairColor + ".png";
  var ready = await preloadIMG(img);
  ...
  if (customSucrette.avatar.customSkin != null) {
    ...
    isHidden = clothe.filter((v) => {
      return v.src == isHidden;
    });
    isHidden = info.filter((v) => {
      return v.groupId == isHidden[0].groupId;
    });
    if (isHidden[0].hide != undefined) {
      for (m = 0; m < isHidden[0].hide.length; m++) {
        if (isHidden[0].hide[m] == "underwears") {
          isHidden = true;
          break;
        }
      }
  ...
  var hayDress = customSucrette.clothe.filter((v) => {
    return v.category == "Dress";
  });
  if (hayDress.length == 1) {
    isHidden = true;
  }

  if (size == "face") {
    var busca = customSucrette.clothe.filter((v) => {
      return v.category == "underwears";
    });
    var img = busca[0].src;
    heelsOn ? (img = img.replace("thumb", "heels")) : (img = img.replace("thumb", "normal"));
  ...
  // Cargar cabello
  var buscaWig = customSucrette.clothe.filter((v) => {
    return v.category == "wig";
  });
  if (buscaWig.length == 0 || size == "face") {
    // Si hay
    var img =
      baseIMG + customSucrette.avatar.hairStyle + "_" + customSucrette.avatar.hairColor + ".png";
    var ready = await preloadIMG(img);
  ...
  if (size != "face") {
    var ropa = customSucrette.clothe.filter((v) => {
      return v.category != "";
    });
...
function updateCustom(img) {
  var categoria = clothe.filter((v) => {
    return v.src == img;
  });
  categoria = info.filter((v) => {
    return v.groupId == categoria[0].groupId;
  });
  ...
  // Obtener máximo
  var max = customSucrette.maxCategory.filter((v) => {
    return v.category == categoria;
  });
  max = max[0].max;

  // Si supera el maximo checkar que hacer
  var caseMax = customSucrette.maxCategory.filter((v) => {
    return v.category == categoria;
  });
  caseMax = caseMax[0].type;

  // Buscar existentes
  var existe = customSucrette.clothe.filter((v) => {
    return v.category == categoria;
  });
  if (categoria == "skin") {
    existe.length = 0;
  ...
  if (existe.length < max) {
    ...
    if (categoria == "shoes") {
      var hayHeels = customSucrette.clothe.filter((v) => {
        return v.category == "highHeels";
      });
      hayHeels.length == 1 ? (caseMax = "replaceHeels") : (caseMax = "add");
    } else if (categoria == "highHeels") {
      var hayShoes = customSucrette.clothe.filter((v) => {
        return v.category == "shoes";
      });
  ...
  if (caseMax != "block") {
    ...
    for (x = 0; x < customSucrette.clothe.length; x++) {
      console.log(
        x + "- cat:" + customSucrette.clothe[x].category + ", src:" + customSucrette.clothe[x].src,
      );
    }
    console.log("-------------------------------------------");
      ...
      for (a = 0; a < pages; a++) {
        ...
        if (a != active) {
          $(".custom-pagination").append(
            '<div class="pagination"><div class="page" page-number="' +
              num +
              '">' +
              num +
              "</div></div>",
          ...
          $(".custom-pagination").append(
            '<div class="pagination current"><div class="page selected" page-number="' +
              num +
              '">' +
              num +
              "</div></div>",
      ...
      if (active < 4) {
        ...
        for (a = 0; a < pages; a++) {
          ...
          if (a != active) {
            if (a == 5) {
              $(".custom-pagination").append(
                '<div class="pagination"><div class="page" data-page="' +
                  a +
                  '" page-number="' +
                  num +
                  '">...</div></div>',
              ...
              $(".custom-pagination").append(
                '<div class="pagination current"><div class="page" page-number="' +
                  num +
                  '">' +
                  num +
                  "</div></div>",
...
function drawPanel() {
  ...
  for (c = 0; c < customSucrette.clothe.length; c++) {
    $(".clothes-container").prepend(
      '<div class="cdk-drag row-z-index-manager"><div class="cloth-asset"><img src="' +
        customSucrette.clothe[c].src +
        '"></div></div>',
    );

    var temp = clothe.filter((v) => {
      return v.src == customSucrette.clothe[c].src;
    });
    temp = info.filter((v) => {
      return v.groupId == temp[0].groupId;
    });

    $(".row-z-index-manager")
      .eq(0)
      .append('<div class="cloth-name">' + temp[0].name + "</div>");
    $(".row-z-index-manager").eq(0).append('<div class="selector-position"></div>');

    $(".selector-position")
      .eq(0)
      .append(
        '<div class="selector up" data-position="' +
          c +
    ...
    $(".selector-position")
      .eq(0)
      .append(
        '<div class="selector down" data-position="' +
          c +
          '"><span class="fa fa-chevron-down"></span></div>',
      );

    if (c == 0) {
      $(".selector.down").eq(0).addClass("disabled");
    }
    if (c == customSucrette.clothe.length - 1) {
      $(".selector.up").eq(0).addClass("disabled");
    }
    if (customSucrette.clothe[c].category != "underwears") {
      $(".row-z-index-manager")
        .eq(0)
        .append(
          '<div class="delete" data-position="' + c + '"><span class="fa fa-times"></span></div>',
        );
    }
  }
}
...
$(function () {
  ...
  $(".showcase").on("click", ".declinable-avatar", function () {
    ...
    if (categoria == "hairStyle") {
      customSucrette.avatar.hairStyle = temp[0];
    } else if (categoria == "hairColor") {
      customSucrette.avatar.hairColor = temp[1];
    } else if (categoria == "eyeType") {
      customSucrette.avatar.eyeType = temp[0];
    } else if (categoria == "eyeColor") {
      customSucrette.avatar.eyeColor = temp[1] + "_no";
    } else if (categoria == "eyebrow") {
      customSucrette.avatar.eyebrow = temp[0];
    } else if (categoria == "mouth") {
      customSucrette.avatar.mouth = temp[0] + "_no";
    } else if (categoria == "makeUp") {
      temp[0] == customSucrette.avatar.makeUp
        ? (customSucrette.avatar.makeUp = null)
        : (customSucrette.avatar.makeUp = temp[0]);
    }
  ...
  $(".download-image").click(function () {
    ...
    // Guardar Sucrette
    $("body").append(
      '<div class="overlay-container"><div class="overlay navigation-menu"></div></div>',
    );
    ...
    $(".overlay.navigation-menu").append(
      '<a class="overlay-button-save size-big active">Grand</a>',
    ); // 820 x 1400
    $(".overlay.navigation-menu").append(
      '<a class="overlay-button-save config" style="left: 20px;"><span class="fa fa-gear"></span></a>',
    ); // boton config

    $(".overlay-container").append('<div class="overlay-popup"></div>');
    $(".overlay-container .overlay-popup").append(
      '<div class="button-close"><span class="fa fa-times"></span></div>',
    );
    $(".overlay-container .overlay-popup").append(
      '<div id="overlay-popup-config"><div> <div class="inner"></div> </div></div>',
    );
    $("#overlay-popup-config div .inner").append(
      '<p>Changer la couleur de fond :</p><p><div id="color-picker-container"><input id="color-picker" type="color"></div></p>',
    );
...
function categoriaNombre() {
  ...
  if (name == "category-HairAccessory") return "Accessoires cheveux";
  if (name == "category-Jacket") return "Vestes";
  if (name == "category-Shirt") return "Hauts";
...
function checkMaxSelect(categoria) {
  ...
  var cat = customSucrette.maxCategory.filter((v) => {
    return v.category == categoria;
  });

  if (cat[0].type != "replace") {
    var count = customSucrette.clothe.filter((v) => {
      return v.category == categoria;
    });
    if (count.length < cat[0].max && cat[0].max != "undefined") {
      return "select";
    } else if (count.length == cat[0].max) {
      return "block";
    } else {
      return "select";
    }
